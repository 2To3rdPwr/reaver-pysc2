...
@gin.configurable
class RandomActorCriticAgent(SyncRunningAgent, ActorCriticAgent):
    """
    This is a really shitty random A2C agent
    Built to test how creating a new agent works
    """
    def __init__(
        self,
        obs_spec: Spec,
    ...
    def loss_fn(self):
        ...
        policy_loss = -1 * np.random.randint(0, tf.reduce_mean(self.policy.logli * adv))
        value_loss = np.random.randint(0, tf.reduce_mean((self.value - returns)**2) * self.value_coef)
        entropy_loss = np.random.randint(0, tf.reduce_mean(self.policy.entropy) * self.entropy_coef)
